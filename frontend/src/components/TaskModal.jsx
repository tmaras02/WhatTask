...
const TaskModal = ({isOpen, onClose, taskToEdit, onSave, onLogout}) => {
    ...
    const handleSubmit = useCallback(async(e)=>{
        ...
        try {
            ...
            console.error(err)      
            setError(err.message || 'An unexpected error occoured in taskmodal')  
        }
        finally{
          setLoading(false)
        }
         ...
         <form onSubmit={handleSubmit} className='space-y-4'>
            ...
            <div>
                 <label className='flex items-center gap-1 text-sm font-medium text-gray-700 mb-1'>
                    <AlignLeft  className='h-4 w-4 text-orange-500'/> Description
                </label>
               ...
               <div>
                  ...
                  <select name='priority' value={taskData.priority} onChange={handleChange} 
                    className={`${baseControlClasses} ${priorityStyles[taskData.priority]}`}>
                      <option>Low</option>
                      <option className='bg-orange-100 text-orange-600'>Medium</option>
                      <option className=' bg-red-100 text-red-600'>High</option>
                  </select>
               </div>
            ...
            <div>
                <label className='flex items-center gap-1 text-sm font-medium text-gray-700 mb-1'>
                    <CheckCircle  className='h-4 w-4 text-orange-500'/> Task Status
                </label>
            ...
            <button type='submit' disabled={loading}
              className='w-full bg-gradient-to-r from-orange-600 to-yellow-400 text-white 
