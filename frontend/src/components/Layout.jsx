                 ...
                 <div className='bg-white rounded-xl p-4 sm:p-5 shadow-sm border border-orange-100'>
                    <div className=' text-base sm:text-lg font-semibold mb-3 sm:mb-4 text-gray-800 flex flex-col items-center gap-2'>
                      ...
                      <div className='grid grid-cols-2 gap-4 sm:gap-4 mb-4 sm:mb-6'>
                            <StatCard title='total Tasks' value={stats.totalCount} icon={<Circle className='w-3.5 h-3.5 sm:w-4 text-orange-500' />} />
                             <StatCard title='Completed' value={stats.completedTasks} icon={<Circle className='w-3.5 h-3.5 sm:w-4 text-green-500' />} />
                              <StatCard title='Pending' value={stats.pendingCount} icon={<Circle className='w-3.5 h-3.5 sm:w-4 text-orange-500' />} />
                               <StatCard title='Completion Rate' value={`${stats.completionPercentage} %`} icon={<Zap className='w-3.5 h-3.5 sm:w-4 text-orange-500' />} />
                      </div>

                      <hr  className='my-3 sm:my-4 border-orange-200'/>

                      <div className='space-y-2 sm:space-y-3'>
                         <div className='flex items-center justify-between text-gray-700'>
                            <span className='text-xs sm:text-sm font-medium flex items-center gap-1.5'>
                                <Circle className='w-2.5 h-2.5 sm:w-3 sm:h-3 text-orange-500 fill-orange-500' />
                                Task Progress
                            </span>
                 ...
                 <div className='bg-white rounded-xl p-4 sm:p-5 shadow-sm border border-orange-100'>
                     ...
                     <div className='space-y-2 sm:space-y-3'>
                        {tasks.slice(0,3).map((task)=>(
                            <div key={task._id || task.id} className='flex items-center justify-between p-2 sm:p-3 hover:bg-orange-50/50 rounded-lg transition-colors duration-200 border border-transparent hover:border-orange-100'>
                        ...
                        {tasks.length === 0 && (
                            <div className='text-center py-4 sm:py-6 px-2'>
                                 <div className='w-12 h-12 sm:w-16 sm:h-16 mx-auto sm:mb-4 rounded-full bg-orange-200 flex items-center justify-center'>
