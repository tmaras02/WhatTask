...
export async function registeredUser(req, res) {
    ...
    if (password.length < 8){
        return res.status(400).json({success: false, message: "Password must contain atleast 8 charachters"});
    }
...
export async function updateProfile(req, res) {
    ...
    if(!name || !email || !validator.isEmail(email)){
     ...
     try{
        ...
        if(exists){
            res.status(409).json({success: false, message: "Email already in use by other account"});
        }
